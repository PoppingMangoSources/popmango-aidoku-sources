...
impl BrowseParams {
	pub fn new(sortby: &str, page: i32) -> Self {
	...
	fn select(&self) -> serde_json::Value {
		...
		if !self
			.content_ratings
			.iter()
			.any(|rating| rating == "pornographic")
		{
...
pub fn parse_latest_uploads(
	...
	let comics = items
		...
		.filter(|comic| comic.translated_language.as_deref() == Some("en"))
...
pub fn parse_browse(response: Response, params: &BrowseParams) -> Result<Vec<ComicData>> {
...
pub fn browse(base_url: &str, params: &BrowseParams) -> Result<Vec<ComicData>> {
	parse_browse(browse_request(base_url, params)?.send()?, params)
}
