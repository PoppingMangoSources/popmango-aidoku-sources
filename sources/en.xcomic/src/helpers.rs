use crate::models::{ChapterData, ComicData, NamedNode};
use aidoku::{
...
fn node_names(nodes: Vec<NamedNode>) -> Vec<String> {
	nodes
		.into_iter()
...
fn content_rating(rating: Option<&str>, genres: &[String]) -> ContentRating {
	if is_pornographic(rating, Some(genres)) {
		...
		|| genres
			.iter()
			.any(|genre| SUGGESTIVE_GENRES.contains(&genre.trim().to_ascii_lowercase().as_str()))
	{
...
pub fn chapter_from_data(mut data: ChapterData, base_url: &str) -> Option<Chapter> {
